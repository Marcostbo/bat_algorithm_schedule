...
class Leitura(object):
    ...
    def __init__(self, path_vazao, path_manutencao, path_rfo):
        self.vaz_afl = None
        self.vaz_hist = None
        self.dr_man = None
        self.rfo_mes = None
    ...
    def ler_dr_man(self, path):
        ...
        dm_sort = np.zeros(shape=(50, 9))

        for im in range(9):
            for it in range(50):
                dm_sort[it, im] = int(a[it, 9 - im - 1])

        self.dr_man = dm_sort

    def ler_rfo(self, path):
        ...
        for tb in range(Num_Turbinas):
            for mes in range(Num_Meses):
                if mes == 0:
                    ...
                    for i in range(aux):
                        num_sorteado = random.randrange(0, num_dias_mes[mes] - 1)
                        matriz_ano[tb, num_sorteado] = 1
                else:
                    aux = int(dias_RFO[mes][tb])
                    for i in range(aux):
                        inicio = fim_dias_mes[mes - 1]
                        fim = fim_dias_mes[mes] - 1
                        num_sorteado = random.randrange(inicio, fim)
                        matriz_ano[tb, num_sorteado] = 1
...
class Volume_Turbinado(object):
    ...
    def vt_individual(self, vaz_afl, lista_turbinas):
        ...
        for t, valor in enumerate(lista_turbinas):
            lista_aux_vaz = []  # lista auxiliar com a vazao turbina pra todos os dias de uma mesma turbina
            if (valor["Grupo"] == "4_pas"):
                for d in range(Num_Dias):
                    lista_aux_vaz.append(
                        int(self.vaz_max_ug(4, vaz_afl[d], self.queda_bruta(vaz_afl[d]))) if self.vaz_max_ug(4, vaz_afl[d],
                                                                                              self.queda_bruta(
                                                                                                  vaz_afl[
                                                                                                      d])) > 0 else 0)
    ...
    def queda_bruta(self, vazao):  # em funcao da vazao afluente/ barramento todo
        qb = self.n_mon(vazao) - self.n_jus(vazao)
        return round(qb, 3)

    def v_turb(self, turb, queda_b, ger):  # ger-> geracao do eixo (calculada em outro momento)
        g_1 = []
        g_2 = []
        if turb == 5:
    ...
    def p_carga(self, vazao):  # vazao turbinada Iindividual (obs: nao é afluente)
    ...
    def p_eixo_max(self, turb, queda_b):
        delta = 0.3
        intera = 0
        while abs(self.p_carga(self.v_turb(turb, queda_b - delta, self.p_e_max(turb,
                                                                queda_b - delta))) - delta) > 0.00001 and queda_b - delta >= 8.8 and intera < 10000:
